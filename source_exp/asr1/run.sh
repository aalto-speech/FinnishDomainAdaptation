...
train_config=aaltoconf/train_large_transformer.yaml
lm_config=aaltoconf/lm_transformer.yaml
decode_config=aaltoconf/decode_beam_40_lm5_ctc3.yaml

# rnnlm related
use_wordlm=false     # false means to train/use a character LM
lm_resume=          # specify a snapshot file to resume LM training
lmtag="kielipankki_5000k_lm"         # tag for managing LMs
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    echo "All cleaned starting"
    for x in $train_set; do
        steps/make_fbank_pitch.sh --cmd "$train_cmd" --nj 32 --write_utt2num_frames true \
            data/${x} exp/make_fbank/${x} ${fbankdir}
        utils/fix_data_dir.sh data/${x}
        echo "All cleaned done"
    done
    echo "starting on dev eval"
    ...
    remove_longshortdata.sh --maxframes 6000 data/${train_set}_org data/${train_set}
    remove_longshortdata.sh --maxframes 6000 data/${train_dev}_org data/${train_dev}

    # compute global CMVN
    echo "compute started"
    compute-cmvn-stats scp:data/${train_set}/feats.scp data/${train_set}/cmvn.ark
    echo "Compute cmvn done"
    # dump features for training

    echo "feature dumps started"
...
if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
    ...
    if [ ${use_wordlm} = true ]; then
        ...
        lmdict=${dict}
        mkdir -p ${lmdatadir}
        # text2token.py -s 1 -n 1 -l ${nlsyms} data/parl_kieli5000k \
        # | cut -f 2- -d" " > ${lmdatadir}/parl_kieli5000k.txt
    ...
    ${cuda_cmd} --gpu ${ngpu} ${lmexpdir}/train.log \
        ...
        --train-label ${lmdatadir}/parl_kieli5000k.txt \
...
expdir=exp/all_cleaned_pytorch_train_large_transformer_retrained_transformer_model

if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    echo "stage 5: Decoding with avg checkpoints, with LM 2048 unit and decode beam size 40"
    nj=64
    ...
    for rtask in ${recog_set}; do
    (
        decode_dir=decode_${rtask}_$(basename ${decode_config%.*})_${lmtag}_duplicate_test
        if [ ${use_wordlm} = true ]; then
