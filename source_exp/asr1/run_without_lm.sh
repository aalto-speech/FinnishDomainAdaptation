...
train_config=aaltoconf/fixed_epoch_train.yaml
...
lmexpname=train_rnnlm_${backend}_large_lm_2048
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    echo "stage 5: Decoding with large LM 2048 unit and beam size 40"
    nj=32
    ...
    (
        ...
        ${decode_cmd} ${expdir}/${decode_dir}/log/decode.33.log \
            asr_recog.py \
            --config ${decode_config} \
            --ngpu ${ngpu} \
            --backend ${backend} \
            --recog-json ${feat_recog_dir}/split${nj}utt/left.json \
            --result-label ${expdir}/${decode_dir}/left.json \
            --model ${expdir}/results/${recog_model}  \
            ${recog_opts}
