...
lm_config=studentconf/lm_unit_2048.yaml
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
   ...
   # Generate the fbank features; by default 80-dimensional fbanks with pitch on each frame
    ...
    #remove_longshortdata.sh --maxframes 10000 --maxchars 2000 data/${train_set}_org data/${train_set}
...
lmexpdir=exp/train_rnnlm_pytorch_kielipankki_10M_lm
mkdir -p ${lmexpdir}
...
expdir=exp/conv-fin-sanasto_train_org_perturbed_pytorch_train_transformer_24l_no_preprocess
mkdir -p ${expdir}
...
#add --nbest to decode config for multiple hypothesis
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    echo "stage 5: Decoding using a LM "
    nj=10
    ...
    for rtask in ${recog_set}; do
    (
        decode_dir=decode_${rtask}_$(basename ${decode_config%.*})_${lmtag}_kieli_10M
        #if [ ${use_wordlm} = true ]; then
